...
export function checkEnv(suppress = false, quiet = false): boolean {
  ...
  if (fileExists && wordCount > 0 && isCurrent) {
    ...
    const templateStr: string = fs.readFileSync(template, 'utf8');
    fs.writeFileSync(
      environmentTs,
...
function isRemoteSet(suppress: boolean, quiet: boolean, envStr = '$SERVER$'): boolean {
