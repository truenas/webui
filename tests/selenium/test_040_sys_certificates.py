...
def test_01_nav_system_certificates(wb_driver):
    wb_driver.find_element_by_xpath(xpaths['submenuCertificates']).click()
    # get the ui element
    ui_element = wb_driver.find_element_by_xpath(xpaths['breadcrumbBar'])
    # get the weather data
    page_data = ui_element.text
    # assert response
    assert "Certificates" in page_data, page_data
    # taking screenshot
