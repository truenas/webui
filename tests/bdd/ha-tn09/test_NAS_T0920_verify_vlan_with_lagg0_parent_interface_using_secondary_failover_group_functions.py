...
def select_disable_failover_and_click_save_check_confirm_on_the_warning_dialog_and_press_ok(driver):
    ...
    if 'mat-checkbox-checked' in class_attribute:
        wait_on_element(driver, 0.5, 4, 'xpath', '//h1[contains(.,"Disable Failover")]')
...
def for_type_select_link_aggregation_for_name_enter_lagg0(driver):
    ...
    wait_on_element(driver, 0.5, 5, 'xpath', '//mat-option[@ix-auto="option__Type_Link Aggregation"]')
...
def for_lagg_protocol_select_lacp_for_lagg_interfaces_select_cxl0_and_cxl1(driver):
    ...
    wait_on_element(driver, 0.5, 5, 'xpath', '//mat-option[@ix-auto="option__Lagg Protocol_LACP"]')
    driver.find_element_by_xpath('//mat-option[@ix-auto="option__Lagg Protocol_LACP"]').click()
    wait_on_element(driver, 0.5, 5, 'xpath', '//mat-select[@ix-auto="select__Lagg Interfaces"]')
    driver.find_element_by_xpath('//mat-select[@ix-auto="select__Lagg Interfaces"]').click()
    wait_on_element(driver, 0.5, 5, 'xpath', '//mat-option[@ix-auto="option__Lagg Interfaces_cxl0"]')
...
def press_apply(driver):
    """Press Apply."""
    wait_on_element(driver, 0.5, 5, 'xpath', '//button[@ix-auto="button__APPLY"]')
...
def finally_you_should_see_a_message_indicating_that_network_changes_have_been_applied_and_you_should_be_able_to_close(driver):
    ...
    wait_on_element(driver, 0.5, 30, 'xpath', '//h1[contains(.,"Changes Saved")]')
...
def for_type_select_link_vlan_for_name_enter_vlan1043(driver):
    ...
    driver.find_element_by_xpath('//mat-select[@ix-auto="select__Type"]').click()
    wait_on_element(driver, 0.5, 5, 'xpath', '//mat-option[@ix-auto="option__Type_VLAN"]')
...
def select_critical_for_failover_group_select_2(driver):
    ...
    wait_on_element(driver, 0.5, 5, 'xpath', '//mat-option[@ix-auto="option__Failover Group_2"]')
...
def for_parent_interface_select_lagg0_created_by_previous_step(driver):
    ...
    wait_on_element(driver, 0.5, 5, 'xpath', '//mat-option[@ix-auto="option__Parent Interface_lagg0: lagg for functional testing"]')
...
def logout_and_log_back_in(driver):
    ...
    time.sleep(0.5)
    driver.find_element_by_xpath('//mat-list-item[@ix-auto="option__Dashboard"]').click()
    wait_on_element(driver, 0.5, 5, 'xpath', '//button[@ix-auto="button__power"]')
    driver.find_element_by_xpath('//button[@ix-auto="button__power"]').click()
    wait_on_element(driver, 0.5, 5, 'xpath', '//button[@ix-auto="option__Log Out"]')
    driver.find_element_by_xpath('//button[@ix-auto="option__Log Out"]').click()
    time.sleep(4)
    wait_on_element(driver, 1, 5, 'xpath', '//input[@placeholder="Username"]')
    driver.find_element_by_xpath('//input[@placeholder="Username"]').clear()
...
def verify_both_controllers_are_on_dashboard(driver, serial1, serial2):
    ...
    wait_on_element(driver, 1, 60, 'xpath', f'//span[contains(.,"{serial1}")]')
    driver.find_element_by_xpath(f'//span[contains(.,"{serial1}")]')
    wait_on_element(driver, 1, 90, 'xpath', f'//span[contains(.,"{serial2}")]')
...
def ha_status_icon_should_appear_and_it_should_reflect_that_ha_is_enabled_when_clicked(driver):
    ...
    wait_on_element(driver, 0.5, 5, 'xpath', '//button[@ix-auto="button__haStatus"]')
    driver.find_element_by_xpath('//button[@ix-auto="button__haStatus"]').click()
    wait_on_element(driver, 0.5, 5, 'xpath', '//h1[contains(.,"HA Enabled)]')
