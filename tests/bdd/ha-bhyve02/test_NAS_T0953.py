...
def try_login_with_ssh(driver):
    ...
    actions.perform()
    assert wait_on_element(driver, 0.5, 7, '//span[contains(.,"(yes/no/[fingerprint])?")]')
    if is_element_present(driver, '//span[contains(.,"(yes/no/[fingerprint])?")]'):
        actions = ActionChains(driver)
        actions.send_keys('yes', Keys.ENTER)
        actions.perform()
    assert wait_on_element(driver, 0.5, 7, '//span[contains(.,"password:")]')
    actions = ActionChains(driver)
