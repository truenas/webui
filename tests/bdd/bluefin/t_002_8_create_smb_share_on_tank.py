...
def test_create_smb_share_on_tank(driver, nas_ip, root_password, tanksmbpath, tanksmbname, tanksmbdescription, mysmbshare, user, password):
   ...
   # click on sharing and click add
    ...
    global tank_smb_path
    tank_smb_path = tanksmbpath
    ...
    driver.find_element_by_xpath('//ix-explorer//ix-label//label//span[contains(text(),"Path")]//ancestor::ix-explorer/div/input').send_keys(tanksmbpath)
    ...
    driver.find_element_by_xpath('//ix-input//ix-label//label//span[contains(text(),"Name")]//ancestor::ix-input/div/input').send_keys(tanksmbname)
    ...
    driver.find_element_by_xpath('//ix-input//ix-label//label//span[contains(text(),"Description")]//ancestor::ix-input/div/input').send_keys(tanksmbdescription)
    ...
    assert wait_on_element(driver, 10, '//div[contains(.,"mytankshare")]')
    time.sleep(2)
    ...
    smbresults3 = ssh_cmd(cmd, 'root', root_password, nas_ip)
    assert smbresults3['result'], smbresults3['output']
    assert 'testfile' in smbresults3['output'], smbresults3['output']
