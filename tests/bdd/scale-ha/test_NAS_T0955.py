...
def log_out_and_try_to_log_back_in_with_the_old_password_for_that_user(nas_vip):
    """Log out and try to log back in with the old password for that user."""
    time.sleep(1)
    assert ssh_sudo_exptext('ls /', nas_vip, 'ericbsd', 'testing', 'Sorry, try again')
...
def try_to_log_back_in_ssh_with_the_new_password_for_that_user(nas_vip):
    """Try to log back in ssh with the new password for that user."""
    assert ssh_sudo_exptext('ls /', nas_vip, 'ericbsd', 'testing1', 'sbin')
