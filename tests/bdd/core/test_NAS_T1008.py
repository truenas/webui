...
def click_storage_on_the_side_menu_and_click_pools(driver):
    ...
    assert wait_on_element(driver, 1, 7, '//mat-list-item[@ix-auto="option__Storage"]')
...
def click_on_add_acl_item(driver):
    ...
    assert wait_on_element(driver, 1, 7, '//button[@ix-auto="button__ADD ACL ITEM"]')
...
def the_new_acl_item_should_appear(driver):
    ...
    assert wait_on_element(driver, 1, 7, '//mat-select[@ix-auto="select__Who"]/div/div/span[contains(.,"Who")]')
...
def click_on_who_select_user(driver):
    ...
    assert wait_on_element(driver, 0.5, 7, '//mat-option[@ix-auto="option__Who_User"]')
...
def the_user_input_should_appear(driver):
    ...
    assert wait_on_element(driver, 1, 7, '(//input[@placeholder="User"])[2]')
...
def in_user_input_enter_eric_and_select_ericbsd(driver, user_input, user):
    ...
    assert wait_on_element(driver, 0.5, 7, f'//mat-option[@ix-auto="option__{user}"]')
