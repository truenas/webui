...
@scenario('features/NAS-T1016.feature', 'Create a wheel group smb share and verify only wheel group can send file')
def test_create_a_wheel_group_smb_share_and_verify_only_wheel_group_can_send_file(driver):
...
def input_wheelsmbshare_as_name_click_to_enable(driver, smb_name):
...
def input_test_wheel_smb_share_as_the_description_click_summit(driver, description):
    """input "test wheel SMB share" as the description, click Summit."""
...
def the_wheelsmbshare_should_be_added_to_the_windows_shares_list(driver, smb_name):
    """the wheelsmbshare should be added to the Windows Shares list."""
    assert wait_on_element_disappear(driver, 1, 20, '//h6[contains(.,"Please wait")]')
    assert wait_on_element(driver, 1, 7, '//div[contains(.,"Samba")]')
    assert wait_on_element(driver, 1, 7, f'//div[contains(.,"{smb_name}")]')
...
def send_a_file_to_the_share_with_nas_ip_wheelsmbshare_and_ericbsd_testing1(driver, nas_ip, smb_name, user, password):
    """send a file to the share with NAS IP/wheelsmbshare and ericbsd%testing1."""
...
def send_a_file_to_the_share_should_fail_with_nas_ip_wheelsmbshare_and_footesting(driver, nas_ip, smb_name, user, password):
    """send a file to the share should fail with NAS IP/wheelsmbshare and foo%testing."""
