...
def when_the_user_field_expand_down_click_the_edit_button(driver):
    ...
    assert wait_on_element(driver, 0.5, 7, '//button[@ix-auto="button__EDIT_foo"]')
...
def on_the_user_edit_page_change_the_user_name_to_too_foo(driver, name):
    ...
    assert wait_on_element(driver, 1, 7, '//a[contains(.,"Edit")]')
...
def verify_the_full_name_changed_to_too_foo(driver, name):
    ...
    assert wait_on_element(driver, 0.5, 7, '//h4[contains(.,"Email:")]')
    assert wait_on_element(driver, 0.5, 7, f'//span[contains(.,"{name}")]')
