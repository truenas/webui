...
def status_should_be_up_with_the_boot_option_checked(driver):
    ...
    assert wait_on_element(driver, 20, '//div[@id="tarsnaptest_Status"]//span[text()="up"]')
    assert wait_on_element(driver, 5, '//mat-checkbox[@id="tarsnaptest_Boot-checkbox"]')
    assert attribute_value_exist(driver, '//mat-checkbox[@id="tarsnaptest_Boot-checkbox"]', 'class', 'mat-checkbox-checked')
