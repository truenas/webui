...
def click_storage_on_the_side_menu_and_click_pools(driver):
    ...
    assert wait_on_element(driver, 1, 5, '//mat-list-item[@ix-auto="option__Storage"]')
...
def click_on_the_tank_three_dots_button_select_add_dataset(driver, pool):
    ...
    assert wait_on_element(driver, 1, 7, f'//mat-icon[@id="actions_menu_button__{pool}"]')
    driver.find_element_by_xpath(f'//mat-icon[@id="actions_menu_button__{pool}"]').click()
    assert wait_on_element(driver, 1, 7, f'//button[@ix-auto="action__{pool}_Add Dataset"]')
...
def click_on_the_my_ldap_dataset_three_dots_button_select_edit_permissions(driver, dataset_name):
    ...
    assert wait_on_element(driver, 1, 7, f'//mat-icon[@ix-auto="options__{dataset_name}"]')
...
def the_edit_permissions_page_should_open(driver):
    ...
    assert wait_on_element(driver, 0.5, 7, '//h4[contains(.,"Dataset Path")]')
...
def select_ldap_user_for_user_click_on_the_apply_user_checkbox(driver, ldap_user):
    ...
    assert wait_on_element(driver, 0.5, 7, f'//mat-option[@ix-auto="option__{ldap_user}"]')
    driver.find_element_by_xpath(f'//mat-option[@ix-auto="option__{ldap_user}"]').click()
    assert wait_on_element(driver, 1, 7, '//mat-checkbox[@ix-auto="checkbox__Apply User"]/label/div')
...
def select_ldap_user_for_group_name_click_on_the_apply_group_checkbox(driver, ldap_user):
    ...
    assert wait_on_element(driver, 0.5, 7, f'//mat-option[@ix-auto="option__{ldap_user}"]')
    driver.find_element_by_xpath(f'//mat-option[@ix-auto="option__{ldap_user}"]').click()
    assert wait_on_element(driver, 1, 7, '//mat-checkbox[@ix-auto="checkbox__Apply Group"]/label/div')
