...
def enter_an_application_name(driver):
    ...
    assert wait_on_element(driver, 7, xpaths.app_Setup.title('ipfs'))
    ...
    driver.find_element_by_xpath(xpaths.app_Setup.app_Name_Input).send_keys('ipfs-test')
...
def confirm_installation_is_successful_and_the_app_is_active(driver):
    ...
    rsc.Verify_App_Status(driver, "ipfs-test", "ipfs")
