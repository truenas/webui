...
def enter_an_application_name(driver):
    ...
    assert wait_on_element(driver, 7, xpaths.app_Setup.title('machinaris'))
    ...
    driver.find_element_by_xpath(xpaths.app_Setup.app_Name_Input).send_keys('machinaris-test')
...
def under_configure_coins_click_enable_flax_checkbox(driver):
    ...
    assert wait_on_element(driver, 10, xpaths.app_Setup.enable_Flax_Checkbox, 'clickable')
    driver.find_element_by_xpath(xpaths.app_Setup.enable_Flax_Checkbox).click()
...
def confirm_installation_is_successful_and_the_app_is_active(driver):
    ...
    rsc.Verify_App_Status(driver, "machinaris-test", "machinaris")
