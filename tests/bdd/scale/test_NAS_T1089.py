...
def the_user_edit_page_should_open_add_the_root_group_and_click_save(driver):
    ...
    assert wait_on_element(driver, 7, xpaths.addUser.rootGroup_option, 'clickable')
    driver.find_element_by_xpath(xpaths.addUser.rootGroup_option).click()
    driver.find_element_by_xpath(xpaths.addUser.rootGroup_option).send_keys(Keys.TAB)
...
def change_should_be_saved_reopen_the_edit_page_root_group_value_should_be_visible(driver):
    ...
    driver.execute_script("arguments[0].scrollIntoView();", element)
    time.sleep(0.5)
    wait_for_value = wait_for_attribute_value(driver, 5, xpaths.addUser.rootGroup_option, 'class', 'mat-selected')
