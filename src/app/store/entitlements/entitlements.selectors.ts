...
export const selectIsEntitledStrictly = (
  ...
  (entitlements, assumed) => {
    ...
    // A failed load leaves a permissive map behind, which has the same shape as an older
    // middleware for the opposite reason. Denying on it would tag a licensed appliance Premium
    // and drop its audit settings from a create. An assumption is not an answer.
