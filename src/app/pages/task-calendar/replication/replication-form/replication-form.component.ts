...
import {
  WebSocketService, TaskService, KeychainCredentialService, ReplicationService, StorageService, DialogService,
} from 'app/services';
...
export class ReplicationFormComponent {
    ...
    this.ws.call('pool.snapshottask.query').subscribe(
      (res) => {
        for (const i in res) {
          ...
          periodicSnapshotTasksField.options.push({ label, value: res[i].id });
        }
      },
    );
  ...
  afterInit(entityForm) {
    ...
    entityForm.formGroup.controls['ssh_credentials'].valueChanges.subscribe(
      (res) => {
        for (const item of ['target_dataset_PUSH', 'source_datasets_PULL']) {
          const { explorerComponent } = _.find(this.fieldConfig, { name: item }).customTemplateStringOptions;
    ...
    entityForm.formGroup.controls['source_datasets_PUSH'].valueChanges.subscribe((value) => {
      ...
      } else if (this.entityForm.formGroup.controls['ssh_credentials'].value) {
        this.ws.call('zettarepl.datasets_have_encryption', [value, this.isRecursive, 'SSH', this.entityForm.formGroup.controls['ssh_credentials'].value]).subscribe((res) => {
          this.encDatasets = res;
          if (this.entityForm.formGroup.controls['properties'].value) {
            if (this.encDatasets && this.encDatasets.length) {
