...
import {
  DialogService, KeychainCredentialService, WebSocketService, ReplicationService, TaskService, StorageService,
} from '../../../../services';
...
export class ReplicationWizardComponent {
  ...
  step0Init() {
    ...
    this.entityWizard.formArray.controls[0].controls['exist_replication'].valueChanges.subscribe((value) => {
      if (value !== null) {
        ...
        } else if (this.selectedReplicationTask !== undefined && this.selectedReplicationTask !== '') {
    ...
    for (const i of ['source', 'target']) {
      ...
      this.entityWizard.formArray.controls[0].controls[credentialName].valueChanges.subscribe((value) => {
        if (value === 'NEW' && this.entityWizard.formArray.controls[0].controls[datasetFrom].value === 'remote') {
          ...
          const { explorerComponent } = _.find(fieldConfig, { name: datasetName }).customTemplateStringOptions;
  ...
  getSourceChildren(node) {
    const fromLocal = this.entityWizard.formArray.controls[0].controls['source_datasets_from'].value === 'local';
  ...
  getTargetChildren(node) {
    const fromLocal = this.entityWizard.formArray.controls[0].controls['target_dataset_from'].value === 'local';
  ...
  createSSHConnection(activedField) {
    ...
    const conf: DialogFormConfiguration = {
      ...
      async customSubmit(entityDialog) {
