...
describe('Harbor Assistant search result helpers', () => {
  ...
  it('encodes same-origin preview URLs', () => {
    ...
    expect(harborAssistantSearchSameOriginAdminUrl('http://192.168.3.82:4174/api/knowledge/preview?path=/recordings/a.mp4'))
      .toBe('/api/beacon/knowledge/preview?path=/recordings/a.mp4');
    expect(harborAssistantSearchSameOriginAdminUrl('http://192.168.3.82:4174/api/harbor-beacon/knowledge/preview?path=/legacy.mp4'))
      .toBe('/api/beacon/knowledge/preview?path=/legacy.mp4');
    expect(harborAssistantSearchSameOriginAdminUrl('/api/cameras/camera-main/snapshot.jpg'))
      .toBe('/api/beacon/cameras/camera-main/snapshot.jpg');
  ...
  it('classifies and sorts waterfall items across image, text, and video hits', () => {
    ...
    expect(items[0].previewUrl).toBe('/api/beacon/knowledge/preview?path=%2Fmnt%2Fnote.md');
