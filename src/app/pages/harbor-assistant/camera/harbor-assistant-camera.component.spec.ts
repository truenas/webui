...
describe('Harbor Assistant camera component', () => {
  ...
  beforeEach(() => {
    ...
    api = {
      ...
      previewUrl: jest.fn((path: string) => `/api/harbor-assistant/knowledge/preview?path=${encodeURIComponent(path)}`),
    };
  });
  ...
  it('uses snapshot polling instead of long-running MJPEG for stream-only cameras', fakeAsync(() => {
    ...
    expect(liveUrl).toContain('/api/harbor-assistant/cameras/cam-1/snapshot.jpg?ts=');
