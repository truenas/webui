...
describe('CertificateSigningRequestsListComponent', () => {
  ...
  const createComponent = createComponentFactory({
    ...
    providers: [
      ...
      mockProvider(DialogService, {
        ...
        confirmForceDelete: jest.fn(() => {
          return of({ confirmed: true, force: true });
        }),
      }),
  ...
  it('deletes the CSR when Delete is pressed', async () => {
    ...
    expect(spectator.inject(DialogService).confirmForceDelete).toHaveBeenCalledWith({
      ...
      message: 'Are you sure you want to delete "cert_default_0"?',
    });
    expect(spectator.inject(DialogService).jobDialog).toHaveBeenCalled();
    expect(spectator.inject(WebSocketService).job).toHaveBeenCalledWith('certificate.delete', [1, true]);
  });
