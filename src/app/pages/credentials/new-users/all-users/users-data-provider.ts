...
export class UsersDataProvider extends ApiDataProvider<'user.query'> {
  ...
  private currentPage: User[] = [];
  private priorityUserShown = false;
  ...
  constructor(
    ...
    this.originalPageSize = this.pagination.pageSize || 50;
  }

  setPriorityUsername(username: string | null): void {
    this.priorityUsername = username;
    this.priorityUserLoaded = false;
    this.priorityUserShown = false;
  }

  setExpandedRow(user: User): void {
    this.expandedRow = user;
    this.expandedRow$.next(user);
  }
    ...
    this.subscription.add(
      this.countRows().pipe(
        ...
        next: (users: User[]) => {
          ...
          const selectedUser = users.find((user) => user.username === this.priorityUsername) || users[0];
          if (this.priorityUserShown) {
            this.router.navigate(['/credentials', 'users-new', 'view', selectedUser.username]);
          }
          this.setExpandedRow(selectedUser);
          this.priorityUsername = null;
          this.priorityUserShown = true;
        },
        error: (error: unknown) => {
  ...
  getUserFromCurrentPage(username: string): User | undefined {
    return this.currentPage.find((user) => user.username === username);
  }
  ...
  private prepareRegularUsersParams(): QueryParams<User> {
    ...
    const adjustedFilters = [...queryFilters];

    // Always filter out priority user from regular queries when priority username is set
    if (this.priorityUsername) {
      adjustedFilters.push(['username', '!=', this.priorityUsername]);
    }

    // On first page, reduce limit by 1 to make room for priority user
    ...
    return [adjustedFilters, adjustedOptions];
  }
