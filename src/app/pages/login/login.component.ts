...
export class Login {
  ...
  constructor(fb: FormBuilder, public _ws: WebSocketService,
  ...
  ngOnInit() {
    ...
    if (this._ws.username && this._ws.password && this._ws.redirectUrl) {
      this.busy = this._ws.login(this._ws.username, this._ws.password)
  ...
  public onSubmit(values: Object): void {
    ...
    this.failed = false;
    if (this.form.valid) {
      this.busy = this._ws.login(this.username.value, this.password.value)
                      .subscribe((result) => { this.loginCallback(result); });
    }
  }
