...
export class SystemSecurityFormComponent implements OnInit {
  ...
  private checkUsersWithoutTwoFactorAuth(callback: () => void): void {
    forkJoin({
      ...
      next: ({ users }) => {
        const usersWithoutTwoFactor = users.filter((user) => (
          !user.twofactor_auth_configured && !user.locked && !user.password_disabled
          && (user.roles?.length > 0 || user.groups?.length > 0)
        ));

        if (usersWithoutTwoFactor.length > 0) {
          this.showStigWarningDialog(usersWithoutTwoFactor, callback);
        } else {
          callback();
        }
      },
