...
export class EntityEditComponent implements OnInit, OnDestroy {
  ...
  setIps(data: any[]) {
    ...
    for (let i in data) {
      ...
      var arrayControl =
          <FormArray>this.formGroup.get("iscsi_target_portal_ips");
      var arrayModel = <DynamicFormArrayModel>this.formService.findById(
          "iscsi_target_portal_ips", this.conf.formModel);
      this.ip_arr_count += 1;
      this.formService.insertFormArrayGroup(this.ip_arr_count, arrayControl,
                                            arrayModel);
