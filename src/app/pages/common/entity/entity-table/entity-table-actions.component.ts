...
@Component({
  ...
  template : `
    <span *ngFor="let action of entity.getActions(row)">
      <button *ngIf="!entity.conf.isActionVisible || entity.conf.isActionVisible.bind(entity.conf)(action.id, row)" md-mini-fab color="primary" class="md-fab-bottom-right-2" (click)="action.onClick(this.row)">{{ action?.label }}</button>
    </span>
  `
})
