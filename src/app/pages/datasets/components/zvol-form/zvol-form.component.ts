...
import { DatasetCreate, DatasetUpdate } from 'app/interfaces/dataset.interface';
      ...
      next: (pkDatasets) => {
        ...
        if (this.isNew) {
          if (this.encryptedParent && pkDatasets[0].encryption_algorithm) {
            this.form.controls.algorithm.setValue(pkDatasets[0].encryption_algorithm.value);
          }
          this.form.controls.encryption.disable();
        ...
        this.readonlyOptions.unshift({ label: `${inheritTr} (${pkDatasets[0].readonly.rawvalue})`, value: inherit });
        let readonlyValue;
        if (this.isNew) {
        ...
        if (pkDatasets && pkDatasets[0].type === DatasetType.Filesystem) {
          this.syncOptions.unshift({ label: `${inheritTr} (${pkDatasets[0].sync.rawvalue})`, value: inherit });
          this.compressionOptions.unshift({ label: `${inheritTr} (${pkDatasets[0].compression.rawvalue})`, value: inherit });
          this.deduplicationOptions.unshift({ label: `${inheritTr} (${pkDatasets[0].deduplication.rawvalue})`, value: inherit });
          this.volblocksizeOptions.unshift({ label: `${inheritTr}`, value: inherit });
          this.snapdevOptions.unshift({ label: `${inheritTr} (${pkDatasets[0].snapdev.rawvalue})`, value: inherit });
          ...
          this.ws.call('pool.dataset.query', [[['id', '=', parentDataset]]]).pipe(untilDestroyed(this)).subscribe({
            next: (parentDataset) => {
              ...
              this.form.controls.name.setValue(pkDatasets[0].name);
              if (pkDatasets[0].comments) {
                this.form.controls.comments.setValue(pkDatasets[0].comments.value);
              } else {
              ...
              if (
                ...
                this.form.controls.sync.setValue(pkDatasets[0].sync.value);
              }
              ...
              if (pkDatasets[0].compression.source === ZfsPropertySource.Inherited) {
                ...
                this.form.controls.compression.setValue(pkDatasets[0].compression.value);
              }
              ...
              if (
                ...
                this.form.controls.deduplication.setValue(pkDatasets[0].deduplication.value);
              }

              this.form.controls.sync.setValue(pkDatasets[0].sync.value);
              if (pkDatasets[0].compression.value === 'GZIP') {
                this.form.controls.compression.setValue(pkDatasets[0].compression.value + '-6');
              }
              this.form.controls.deduplication.setValue(pkDatasets[0].deduplication.value);
              ...
              if (
                ...
                this.form.controls.snapdev.setValue(pkDatasets[0].snapdev.value);
              }
