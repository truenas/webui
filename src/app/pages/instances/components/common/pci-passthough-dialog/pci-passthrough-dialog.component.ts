...
export class PciPassthroughDialog implements OnInit {
  ...
  private loadDevices(): void {
    this.api.call('container.device.pci_device_choices').pipe(
      map((choices) => {
        const devices: PciPassthroughDevice[] = [];
        const types = new Set<string>();

        Object.entries(choices).forEach(([address, device]) => {
          const pciDevice = device as VirtualizationPciDeviceOption;
          if (pciDevice.error) {
            // TODO: Workaround for a MW issue.
            return;
          }

          types.add(pciDevice.controller_type ?? unknownType);
