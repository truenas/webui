...
describe('ExtentListComponent', () => {
  ...
  it('opens extent form when "Add" button is pressed', async () => {
    ...
    expect(spectator.inject(OldSlideInService).open).toHaveBeenCalledWith(ExtentFormComponent, { wide: true });
  });
  ...
  it('opens extent form when "Edit" button is pressed', async () => {
    ...
    expect(spectator.inject(OldSlideInService).open).toHaveBeenCalledWith(ExtentFormComponent, {
      data: extents[0],
      wide: true,
    });
  });
