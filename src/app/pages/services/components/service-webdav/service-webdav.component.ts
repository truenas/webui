...
export class ServiceWebdavComponent implements OnInit{
  ...
  protected fieldConfig: FieldConfig[] = [
    ...
    },
    {
        ...
        inputType: 'password',
        validation: [
            matchOtherValidator('webdav_password2')
        ]
    },
    {
        type: 'input',
 ...
 constructor(
  ...
  ngOnInit() {
    this.systemGeneralService.getCertificates().subscribe((res) => {
      this.webdav_certssl = _.find(this.fieldConfig, {'name': 'webdav_certssl'});
      res.data.forEach((item) => {
        this.webdav_certssl.options.push({ label: item.cert_common, value: item.id});
      });
    });
  }
