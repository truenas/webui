...
export class ServiceFTPComponent implements OnInit {
  ...
  afterInit(entityEdit: any) {
    ...
    this.bwFields.forEach((field) => entityEdit.formGroup.controls[field].valueChanges.subscribe((value) => {
  ...
  resourceTransformIncomingRestData(data) {
    this.bwFields.forEach((field) => data[field] = this.storageService.convertBytestoHumanReadable(data[field] * 1024, 0, 'KiB'));
  ...
  beforeSubmit(data) {
    this.bwFields.forEach((field) => data[field] = this.storageService.convertHumanStringToNum(data[field]) / 1024);
