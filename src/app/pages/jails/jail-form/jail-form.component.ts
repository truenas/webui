...
// DynamicInputModel, DynamicSelectModel, DynamicRadioGroupModel } from
...
export class JailFormComponent {
  ...
  constructor(protected router: Router, protected jailService: JailService,
  ...
  afterInit(entityForm: any) {
    ...
    this.jail_ipv4_netmask =
        _.find(this.fieldConfig, {'name' : 'jail_ipv4_netmask'});
    this.jail_bridge_ipv4_netmask =
        _.find(this.fieldConfig, {'name' : 'jail_bridge_ipv4_netmask'});
    ipv4_netmask_options.forEach((item) => {
      this.jail_ipv4_netmask.options.push(
          {label : item.label, value : item.value});
      this.jail_bridge_ipv4_netmask.options.push(
          {label : item.label, value : item.value});
    });

    this.jail_ipv6_prefix =
        _.find(this.fieldConfig, {'name' : 'jail_ipv6_prefix'});
    this.jail_bridge_ipv6_prefix =
        _.find(this.fieldConfig, {'name' : 'jail_bridge_ipv6_prefix'});
    ipv6_prefix_options.forEach((item) => {
      this.jail_ipv6_prefix.options.push(
          {label : item.label, value : item.value});
      this.jail_bridge_ipv6_prefix.options.push(
          {label : item.label, value : item.value});
    });
    ...
    this.jailService.listTemplates().subscribe((res) => {
      ...
      res.data.forEach((item) => {
        this.jail_type.options.push(
            {label : item.jt_name, value : item.jt_name});
      });
    });
