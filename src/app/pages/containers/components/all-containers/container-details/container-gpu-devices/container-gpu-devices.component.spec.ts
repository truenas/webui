...
describe('ContainerGpuDevicesComponent', () => {
  ...
  it('navigates to advanced settings and sets pending highlight when clicking enable NVIDIA drivers link', () => {
    spectator.detectChanges();
    const link = spectator.query('.nvidia-drivers-warning a');
    spectator.click(link);

    const searchDirectives = spectator.inject(UiSearchDirectivesService);
    expect(searchDirectives.setPendingUiHighlightElement).toHaveBeenCalledWith(
      nvidiaDriversCardElements.elements.nvidiaDrivers,
    );

    const router = spectator.inject(Router);
    expect(router.navigate).toHaveBeenCalledWith(['/system', 'advanced']);
  });
});
