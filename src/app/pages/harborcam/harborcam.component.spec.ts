...
describe('HarborCamComponent', () => {
  let spectator: Spectator<HarborCamComponent>;
  let snapshotSubject: Subject<HarborBotSnapshotTaskResponse>;
  ...
  beforeEach(() => {
    snapshotSubject = new Subject<HarborBotSnapshotTaskResponse>();
  ...
  it('uses snapshot polling instead of long-running MJPEG for stream-only cameras', fakeAsync(() => {
    ...
    expect(liveUrl).toContain('/api/harbordesk/cameras/cam-1/snapshot.jpg?ts=');
  ...
  it('opens DVR media in an inline viewer instead of a popup', fakeAsync(() => {
    ...
    expect(spectator.query('[data-testid="harborcam-media-viewer"]')).toExist();
    expect(spectator.query('[data-testid="harborcam-media-viewer"] video')).toExist();
  ...
  it('labels media library recordings as videos', fakeAsync(() => {
    ...
    expect(spectator.component.mediaKindLabel(spectator.component.timelineItems()[0])).toBe('视频');
    discardPeriodicTasks();
  }));
  ...
  it('shows a starting recording badge before the start request resolves', fakeAsync(() => {
    const startSubject = new Subject<HarborBotDvrStatusResponse>();
  ...
  it('shows finalizing state and a pending recording card while stopping', fakeAsync(() => {
    api.dvrStatus = jest.fn(() => of(dvrStatus('recording')));
    const stopSubject = new Subject<HarborBotDvrStatusResponse>();
  ...
  it('clears action messages while preserving action errors', fakeAsync(() => {
    ...
    expect(componentState.actionMessage()).toBe('已保留当前预览，后台归档失败。');
...
function dvrStatus(status = 'stopped'): HarborBotDvrStatusResponse {
  return {
    generated_at: '1',
    statuses: [
      {
...
function dvrTimeline(): HarborBotDvrTimelineResponse {
  return {
    generated_at: '1',
...
function searchResponse(): HarborBotSearchResponse {
  return {
    query: '谁倒了啤酒',
    roots: [],
    total_matches: 1,
