...
export class VmListComponent implements EntityTableConfig<VirtualMachineRow>, OnInit, AfterViewInit {
 ...
 the Virtual Machine when it is still active or has an undefined state.'),
  ...
  checkMemory(): void {
    this.ws.call(this.wsMethods.getAvailableMemory).pipe(untilDestroyed(this)).subscribe((memory) => {
      this.availableMemory = this.storageService.convertBytesToHumanReadable(memory);
    });
  }
