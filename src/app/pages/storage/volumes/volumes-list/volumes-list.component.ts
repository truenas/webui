...
export class VolumesListTableConfig implements InputTableConf {
  ...
  route_add = ['storage', 'pools', 'import'];
  ...
  getActions(rowData: any) {
      ...
      actions.push({
        ...
        onClick: (row1) => {
          ...
          async function doDetach() {
            ...
            let title; let warningA; let warningB; let unknownA; let unknownB; let encrypted; let
              sysPoolWarning;
                    ...
                    dialogRef.componentInstance.failure.subscribe((res) => {
                      ...
                      if (res.error) {
                        if (res.exc_info.extra && res.exc_info.extra['code'] === 'control_services') {
                          entityDialog.dialogRef.close(true);
                          dialogRef.close(true);
                          let stopMsg; let restartMsg; let
                            continueMsg;
    ...
    if (rowData.type === 'FILESYSTEM') {
      ...
      actions.push({
        ...
        onClick: (row1) => {
          const datasetParent = row1.id.split('/');
          datasetParent.pop();
          this._router.navigate(new Array('/').concat([
    ...
    if (rowData.type === 'VOLUME') {
      ...
      actions.push({
        ...
        label: T('Edit Zvol'),
        onClick: (row1) => {
          const zvolParent = row1.id.split('/');
          zvolParent.pop();
          this._router.navigate(new Array('/').concat([
