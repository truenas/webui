...
export class SnapshotListComponent {
  ...
  getActions() {
    return [
      ...
      },
      {
        ...
        onClick: (snapshot) => this._router.navigate(new Array('/').concat(['storage', 'snapshots', 'clone', snapshot.name])),
      },
      {
        id: 'rollback',
        icon: 'history',
  ...
  startMultiDeleteProgress(selected) {
    ...
    dialogRef.componentInstance.success.subscribe((job_res) => {
      ...
      let infoMessage = `${T('Deleted')} ${params[1].length} `;
  ...
  rollbackSubmit(entityDialog) {
    const { parent } = entityDialog;
    const item = entityDialog.parent.rollback;
    const { recursive } = entityDialog.formValue;
    const data = {};
    if (recursive !== null) {
      data[recursive] = true;
    }
