...
describe('DeleteZvolDialogComponent', () => {
  let spectator: Spectator<DeleteZvolDialogComponent>;
  let websocket: WebSocketService;
  let loader: HarnessLoader;
  const createComponent = createComponentFactory({
    component: DeleteZvolDialogComponent,
    imports: [
      AppLoaderModule,
      ReactiveFormsModule,
      IxFormsModule,
  ...
  async function confirmAndDelete(): Promise<void> {
    ...
    const deleteButton = await loader.getHarness(MatButtonHarness.with({ text: 'Delete Zvol' }));
    await deleteButton.click();
  }

  it('loads and shows services that use zvol', () => {
  ...
  it('loads and shows known processes that use zvol', () => {
  ...
  it('shows unknown processes that use zvol', () => {
    const processesSection = spectator.query('.unknown-processes');
    expect(processesSection).toHaveText('These unknown processes are using the zvol:');
    const processes = processesSection.querySelectorAll('li');
