...
export class PermissionsSidebarComponent implements OnInit, OnChanges {
  ...
  get editPermissionsUrl(): string[] {
    if (this.acl.trivial) {
      return ['/storage-old/permissions', this.dataset.id];
    }

    if (this.acl.acltype === AclType.Posix1e) {
      return ['/storage-old/id', this.dataset.pool, 'dataset', 'posix-acl', this.dataset.id];
    }

    return ['/storage-old/id', this.dataset.pool, 'dataset', 'acl', this.dataset.id];
  }
