...
} from 'app/pages/storage-old/volumes/create-snapshot-dialog/create-snapshot-dialog.component';
import {
  DeleteDatasetDialogComponent,
} from 'app/pages/storage-old/volumes/delete-dataset-dialog/delete-dataset-dialog.component';
...
} from 'app/pages/storage-old/volumes/encyption-options-dialog/encryption-options-dialog.component';
import { ExportDisconnectModalComponent, ExportDisconnectModalState } from 'app/pages/storage-old/volumes/volumes-list/components/export-disconnect-modal.component';
...
export class VolumesListTableConfig implements EntityTableConfig {
    ...
    if (this.parentVolumesListComponent.hasEncryptedRoot[rowData.name]
      ...
      actions.push({
        ...
        onClick: (row1: VolumesListPool) => {
          ...
          this.ws.call('core.download', ['pool.dataset.export_keys', [row1.name], fileName]).pipe(untilDestroyed(this, 'destroy')).subscribe({
            next: (res) => {
              this.loader.close();
              const url = res[1];
  ...
  getActions(rowData: VolumesListPool | VolumesListDataset): EntityTableAction[] {
    (rowData as VolumesListDataset).is_passphrase = (!!('key_format' in rowData && rowData.key_format.parsed === 'passphrase'));
    ...
    if (rowData.type === 'zpool') {
      ...
      if (rowData.status !== PoolStatus.Offline) {
        actions.push({
          ...
          label: T('Add Vdevs'),
          onClick: (row1: VolumesListPool) => {
            this.router.navigate(['/', 'storage-old', 'manager', row1.id]);
          },
        });
        actions.push({
          id: rowData.name,
        ...
        actions.push({
          ...
          label: T('Status'),
          onClick: (row1: VolumesListPool) => {
            this.router.navigate(['/', 'storage-old', 'status', row1.id]);
          },
        });
        actions.push({
          id: rowData.name,
    ...
    if (rowData.type === DatasetType.Filesystem) {
      ...
      if (rowDataPathSplit[1] !== 'iocage' && !rowData.locked) {
        ...
        },
        {
          ...
          label: T('User Quotas'),
          onClick: () => {
            this.router.navigate(['/', 'storage-old', 'user-quotas', rowData.id]);
          },
        },
        {
          id: rowData.name,
          ...
          onClick: () => {
            this.router.navigate(['/', 'storage-old', 'group-quotas', rowData.id]);
          },
        });
      }
          ...
          encryptionActions.push({
            ...
            onClick: () => {
              ...
              this.router.navigate([
                '/', 'storage-old', 'id', rowData.pool, 'dataset',
                'unlock', rowData.id,
              ]);
            },
          });
            ...
            onClick: () => {
              ...
              dialogRef.componentInstance.success.pipe(untilDestroyed(this, 'destroy')).subscribe((res: Job<string>) => {
                ...
                this.dialogService.confirm({
                  ...
                  this.loader.open();
                  this.ws.call('core.download', ['pool.dataset.export_key', [rowData.id, true], fileName]).pipe(untilDestroyed(this, 'destroy')).subscribe({
                    next: (res) => {
                      this.loader.close();
                      const url = res[1];
                      this.storageService.streamDownloadFile(url, fileName, mimetype)
