...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>{{ '时间范围' | translate }}</h2>
    <mat-dialog-content class="time-range-dialog-content">
      <form class="time-range-dialog-form" [formGroup]="form">
        <div class="preset-row">
          <button mat-button type="button" (click)="setPreset('today')">{{ '今天' | translate }}</button>
          <button mat-button type="button" (click)="setPreset('last24')">{{ '最近 24 小时' | translate }}</button>
          <button mat-button type="button" (click)="setPreset('last7')">{{ '最近 7 天' | translate }}</button>
          <button mat-button type="button" (click)="resetForm()">{{ '全部时间' | translate }}</button>
        </div>

        <section class="time-boundary">
          <h3>{{ '开始时间' | translate }}</h3>
          <div class="time-picker-grid">
            <label>
              <span>{{ '年' | translate }}</span>
              <select formControlName="fromYear">
                <option value="">{{ '年' | translate }}</option>
            ...
            <label>
              <span>{{ '月' | translate }}</span>
              <select formControlName="fromMonth">
                <option value="">{{ '月' | translate }}</option>
            ...
            <label>
              <span>{{ '日' | translate }}</span>
              <select formControlName="fromDay">
                <option value="">{{ '日' | translate }}</option>
            ...
            <label>
              <span>{{ '时' | translate }}</span>
              <select formControlName="fromHour">
                <option value="">{{ '时' | translate }}</option>
            ...
            <label>
              <span>{{ '分' | translate }}</span>
              <select formControlName="fromMinute">
                <option value="">{{ '分' | translate }}</option>
    ...
    <mat-dialog-actions align="end">
      <button mat-button type="button" (click)="clear()">{{ '清除' | translate }}</button>
      <button mat-button type="button" (click)="close()">{{ '取消' | translate }}</button>
      <button mat-flat-button color="primary" type="button" (click)="apply()">{{ '应用' | translate }}</button>
    </mat-dialog-actions>
  `,
  styles: [`
