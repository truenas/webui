...
describe('CronSchedulePreview - listNextRunsInMonth', () => {
  function toNewYorkTime(date: Date): string {
    return format(utcToZonedTime(date, 'America/New_York'), 'yyyy-MM-dd HH:mm:ss');
  }
  ...
  describe('Every Minute - * * * * *', () => {
    ...
    it('lists next runs for remaining days in current month', () => {
      const nextRuns = cron.listNextRunsInMonth('2022-02-19 10:45:02', 3);
    ...
    it('lists runs from start of the month next month', () => {
      const nextRuns = cron.listNextRunsInMonth('2022-03-01 00:00:00', 3);
    ...
    it('lists last runs in the month', () => {
      const nextRuns = cron.listNextRunsInMonth('2022-02-28 23:58:00', 3);
  ...
  describe('Every hour - 0 * * * *', () => {
    ...
    it('lists run from start of the month next month', () => {
      const nextRuns = cron.listNextRunsInMonth('2022-03-01 0:00:00', 3);
    ...
    it('lists last runs in the month', () => {
      const nextRuns = cron.listNextRunsInMonth('2022-02-28 22:00:00', 5);
  ...
  describe('Every day - 0 0 * * *', () => {
    ...
    it('lists last runs in a month', () => {
      const nextRuns = cron.listNextRunsInMonth('2022-02-27 0:00:00', 3);
  ...
  describe('Every Week - 15 9 * * mon', () => {
    ...
    it('lists last sunday in the month', () => {
      const nextRuns = cron.listNextRunsInMonth('2022-02-27 9:15:00', 3);
