...
describe('TruenasLogoComponent', () => {
  ...
  it('community-edition: shows a logotype', async () => {
    ...
    expect(await mark.getName()).toBe('app-truenas-logo-mark-color');
    expect(await text.getName()).toBe('app-truenas-logo-type-color');
  });
  ...
  it('community-edition: shows full logo in white', async () => {
    ...
    icons = await loader.getAllHarnesses(TnIconHarness);
    expect(await icons[0].getName()).toBe('app-truenas-logo-ce');
  });
  ...
  it('enterprise: shows full logo in color', async () => {
    ...
    expect(await icons[0].getName()).toBe('app-truenas-logo-enterprise-color');
  });
  ...
  it('checks white color', async () => {
    ...
    expect(await mark.getName()).toBe('app-truenas-logo-mark');
    expect(await text.getName()).toBe('app-truenas-logo-type');
  });
  ...
  it('checks when logo text is hidden', async () => {
    ...
    expect(await icons[0].getName()).toBe('app-truenas-logo-mark-color');
  });
});
