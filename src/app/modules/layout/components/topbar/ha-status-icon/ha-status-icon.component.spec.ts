...
import { HaStatusIconComponent } from 'app/modules/layout/components/topbar/ha-status-icon/ha-status-icon.component';
import {
...
describe('HaStatusIconComponent', () => {
  ...
  it('shows an icon when HA is enabled', () => {
    expect(spectator.query('ix-icon')).toHaveAttribute('name', 'ix:ha_enabled');
  });
  ...
  it('shows a reconnecting icon when HA disabled reason is NoSystemReady', () => {
    ...
    expect(spectator.query('ix-icon')).toHaveAttribute('name', 'ix:ha_reconnecting');
  });
  ...
  it('shows an HA disabled icon when HA is disabled', () => {
    ...
    expect(spectator.query('ix-icon')).toHaveAttribute('name', 'ix:ha_disabled');
  });
