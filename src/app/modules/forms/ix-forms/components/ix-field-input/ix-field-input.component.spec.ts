...
import { IxFieldInputComponent } from './ix-field-input.component';

describe('IxFieldInputComponent', () => {
  let spectator: SpectatorHost<IxFieldInputComponent>;
  const formControl = new FormControl<unknown>();
  const blurFormControl = new FormControl('', { updateOn: 'blur' });
  let loader: HarnessLoader;

  const createHost = createHostFactory({
    component: IxFieldInputComponent,
    imports: [
      ReactiveFormsModule,
    ],
  });
  ...
  describe('default `updateOn: change` strategy', () => {
    ...
    describe('types', () => {
      ...
      it('renders input element as pseudo-password field (via text input type) to disable password managers', () => {
        formControl.setValue('test');
  ...
  describe('non-default but `updateOn: blur` strategy', () => {
    beforeEach(() => {
      spectator = createHost('<ix-field-input [formControl]="blurFormControl"></ix-field-input>', {
