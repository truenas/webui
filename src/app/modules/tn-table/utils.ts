...
/**
 * Workarounds for behavior `tn-table` doesn't expose yet. Deliberately kept out of
 * `app/modules/ix-table`, which NAS-141021 is retiring: these reference nothing from it and will
 * outlive it, and each is meant to be deleted the moment `@truenas/ui-components` grows the input
 * it stands in for.
 *
 * Bridges between the two table models (`mapTnSortToTableSort`, `toDisplayedColumns`, the
 * `dataProvider*` adapters) stay in `app/modules/ix-table/utils.ts` — they die with ix-table.
 */
