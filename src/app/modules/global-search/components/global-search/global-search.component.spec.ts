...
describe('GlobalSearchComponent', () => {
  ...
  it('should update search results for "Filtered" input', fakeAsync(() => {
    ...
    expect(spectator.component.searchResults).toEqual([{ hierarchy: ['Filtered Result 1'], requiredRoles: [Role.FullAdmin] }]);
    expect(spectator.component.searchResults).toHaveLength(1);
  }));

  it('should handle empty search results for "Unknown" input', fakeAsync(() => {
    const inputElement = spectator.query('.search-input');
    ...
    expect(spectator.component.searchResults).toHaveLength(0);
  }));
