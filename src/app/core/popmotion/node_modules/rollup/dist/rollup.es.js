/*
	Rollup.js v0.59.4
	Mon, 28 May 2018 04:13:19 GMT - commit 5eb154d53ea59c7847b4539ab6cbe9ec47469533
...
*/
import path, { dirname, relative, basename, extname, resolve, sep } from 'path';
...
var NamespaceVariable = /** @class */ (function (_super) {
    ...
    NamespaceVariable.prototype.renderBlock = function (options) {
        ...
        var members = Object.keys(this.originals).map(function (name) {
            ...
            if ((_this.referencedEarly || original.isReassigned) && !options.legacy) {
                return options.indent + "get " + name + " () { return " + original.getName() + "; }";
            }
            if (options.legacy && reservedWords$1.indexOf(name) !== -1)
                name = "'" + name + "'";
            return "" + options.indent + name + ": " + original.getName();
        });
        var name = this.getName();
        var callee = options.freeze
        ...
        if (options.namespaceToStringTag) {
            output += "\nif (typeof Symbol !== 'undefined' && Symbol.toStringTag)\n" + options.indent + "Object.defineProperty(" + name + ", Symbol.toStringTag, { value: 'Module' });\nelse\n" + options.indent + "Object.defineProperty(" + name + ", 'toString', { value: function () { return '[object Module]' } });\n" + callee + "(" + name + ");";
        }
...
var chars$1 = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ$_';
...
var Import$1 = /** @class */ (function (_super) {
    ...
    Import.prototype.render = function (code, options) {
        ...
        if (this.resolutionNamespace) {
            code.overwrite(this.parent.start, this.parent.end, "Promise.resolve().then(function () { return " + this.resolutionNamespace + "; })");
            return;
        }
        var importMechanism = dynamicImportMechanisms[options.format];
        if (importMechanism) {
...
var globalImportMetaUrlMechanism = "(typeof document !== 'undefined' ? document.currentScript && document.currentScript.src || location.href : new URL('file:' + __filename).href)";
...
var MetaProperty$1 = /** @class */ (function (_super) {
    ...
    MetaProperty.prototype.renderImportMetaMechanism = function (code, importMetaProperty, format) {
        if (format === 'system')
            code.overwrite(this.meta.start, this.meta.end, 'module');
        else if (importMetaProperty === 'url')
            return importMetaUrlMechanisms[format];
    };
    return MetaProperty;
}(NodeBase));
...
var MemberExpression$1 = /** @class */ (function (_super) {
    ...
    MemberExpression$$1.prototype.render = function (code, options, _a) {
        ...
        else if (this.object instanceof MetaProperty$1 && this.object.meta.name === 'import') {
            this.context.hasImportMeta = true;
            var importMetaMechanism = this.object.renderImportMetaMechanism(code, this.property instanceof Identifier$1 && this.property.name, options.format);
            if (importMetaMechanism)
                code.overwrite(this.start, this.end, importMetaMechanism);
        }
        else {
            if (isCalleeOfDifferentParent) {
                code.appendRight(this.start, '0, ');
...
var Module = /** @class */ (function () {
    ...
    Module.prototype.traceVariable = function (name) {
        ...
        if (name in this.imports) {
            ...
            if (!declaration) {
                this.graph.handleMissingExport(importDeclaration.name, this, otherModule.id, importDeclaration.start);
            }
            return declaration;
        }
        return null;
    ...
    Module.prototype.traceExport = function (name) {
        ...
        if (reexportDeclaration) {
            ...
            if (!declaration) {
                this.graph.handleMissingExport(reexportDeclaration.localName, this, reexportDeclaration.module.id, reexportDeclaration.start);
            }
            return declaration;
        }
        ...
        if (exportDeclaration) {
            ...
            var declaration = this.traceVariable(name_1);
...
function system(magicString, _a) {
    ...
    var wrapperStart = "System.register([" + dependencyIds.join(', ') + "], function (exports, module) {\n" + t + "'use strict';" + starExcludesSection + importBindingsSection + "\n" + t + "return {" + (setters.length
        ? "\n" + t + t + "setters: [" + setters
            .map(function (s) { return (s ? "function (module) {\n" + t + t + t + s + "\n" + t + t + "}" : "function () {}"); })
            .join(', ') + "],"
...
function getExportBlock(exports, dependencies, namedExportsMode, interop, mechanism) {
            ...
            reexports.forEach(function (specifier) {
                if (specifier.imported === 'default' && !isChunk) {
                    ...
                    if (exportsNamesOrNamespace) {
                        exportBlock += (exportBlock ? '\n' : '') + "exports." + specifier.reexported + " = " + name + (interop !== false ? '__default' : '.default') + ";";
                    }
                    else {
...
function amd(magicString, _a, options) {
    var graph = _a.graph, namedExportsMode = _a.namedExportsMode, hasExports = _a.hasExports, indentString = _a.indentString, intro = _a.intro, outro = _a.outro, dynamicImport = _a.dynamicImport, importMeta = _a.importMeta, dependencies = _a.dependencies, exports = _a.exports, isEntryModuleFacade = _a.isEntryModuleFacade;
    ...
    if (intro)
        magicString.prepend(intro);
    var exportBlock = getExportBlock(exports, dependencies, namedExportsMode, options.interop);
    if (exportBlock)
        magicString.append('\n\n' + exportBlock);
...
function cjs(magicString, _a, options) {
    ...
    var exportBlock = getExportBlock(exports, dependencies, namedExportsMode, options.interop, 'module.exports =');
    magicString.prepend(intro);
    if (exportBlock)
        magicString.append('\n\n' + exportBlock);
    if (outro)
...
function esm(magicString, _a) {
    ...
    var importBlock = dependencies
        ...
        if (imports) {
            ...
            if (starImport_1) {
                output += "import * as " + starImport_1.local + " from '" + id + "';";
                if (imports.length > 1)
                    output += '\n';
            }
            if (defaultImport_1 && imports.length === 1) {
                output += "import " + defaultImport_1.local + " from '" + id + "';";
            }
...
function iife(magicString, _a, options) {
    var graph = _a.graph, namedExportsMode = _a.namedExportsMode, hasExports = _a.hasExports, indentString = _a.indentString, intro = _a.intro, outro = _a.outro, dependencies = _a.dependencies, exports = _a.exports;
...
function umd(magicString, _a, options) {
    ...
    var define = amdOptions.define || 'define';
    var cjsExport = !namedExportsMode && hasExports ? "module.exports = " : "";
...
var Chunk$1 = /** @class */ (function () {
    ...
    Chunk.prototype.getModuleIds = function () {
        return this.orderedModules.map(function (module) { return module.id; });
    };
    ...
    Chunk.prototype.generateInternalExports = function (options) {
        if (this.isEntryModuleFacade)
            return;
        var mangle = options.format === 'system' || options.format === 'es';
    ...
    Chunk.prototype.preRender = function (options, inputBase) {
        ...
        for (var _d = 0, _e = this.orderedModules; _d < _e.length; _d++) {
            var module = _e[_d];
        ...
        if (hoistedSource)
            magicString.prepend(hoistedSource + '\n\n');
    ...
    Chunk.prototype.generateNamePreserveModules = function (preserveModulesRelativeDir) {
        return (this.id = normalize(relative(preserveModulesRelativeDir, this.entryModule.id)));
    };
    Chunk.prototype.generateName = function (pattern, addons, options, existingNames) {
        var _this = this;
        // replace any chunk replacements
        var outName = pattern.replace(/\[(hash|alias)\]/g, function (type) {
            switch (type) {
                case '[hash]':
    ...
    Chunk.prototype.render = function (options, addons) {
        ...
        var hasImportMeta = this.orderedModules.some(function (module) { return module.hasImportMeta; });
        ...
        return transformBundle(prevCode, this.graph.plugins, bundleSourcemapChain, options).then(function (code) {
            if (options.sourcemap) {
                ...
                if (_this.graph.hasLoaders ||
                    ...
                    map = collapseSourcemaps(_this, file, decodedMap, _this.usedModules, bundleSourcemapChain);
                }
                else {
...
var Graph = /** @class */ (function () {
    ...
    Graph.prototype.loadEntryModules = function (entryModules, manualChunks) {
        ...
        if (Array.isArray(entryModules)) {
            entryModuleAliases = entryModules.map(function (id) { return nameWithoutExtension(basename(id)); });
    ...
    Graph.prototype.buildChunks = function (entryModules, manualChunks, preserveModules) {
        // Phase 1 – discovery. We load the entry module and find which
        // modules it imports, and import those, until we have all
        // of the entry module's dependencies
        var _this = this;
        ...
        return this.loadEntryModules(entryModules, manualChunks).then(function (_a) {
            ...
            _this.link();
            var _b = _this.analyseExecution(entryModules, !preserveModules, manualChunkModules), orderedModules = _b.orderedModules, dynamicImports = _b.dynamicImports, dynamicImportAliases = _b.dynamicImportAliases;
            ...
            for (var _g = 0, chunkList_1 = chunkList; _g < chunkList_1.length; _g++) {
                var chunk = chunkList_1[_g];
                chunk.link();
            }
            ...
            for (var _h = 0, chunkList_2 = chunkList; _h < chunkList_2.length; _h++) {
                var chunk = chunkList_2[_h];
            ...
            if (!preserveModules) {
                for (var _j = 0, entryModules_2 = entryModules; _j < entryModules_2.length; _j++) {
                    var entryModule = entryModules_2[_j];
    ...
    Graph.prototype.analyseExecution = function (entryModules, graphColouring, chunkModules) {
        var _this = this;
        ...
        var fetchDynamicImportsPromise = !this.dynamicImport
            ...
            : Promise.all(module.getDynamicImportExpressions().map(function (dynamicImportExpression, index) {
                return Promise.resolve(_this.resolveDynamicImport(dynamicImportExpression, module.id)).then(function (replacement) {
                    ...
                    var alias = nameWithoutExtension(basename(replacement));
...
function collectAddon(graph, initialAddon, addonName, sep$$1) {
    ...
    return runSequence([
        ...
        .concat(graph.plugins.map(function (plugin, idx) {
        return {
...
function rollup(rawInputOptions) {
    try {
        ...
        timeStart('BUILD', 1);
        var codeSplitting = (inputOptions_1.experimentalCodeSplitting && typeof inputOptions_1.input !== 'string') ||
            inputOptions_1.experimentalPreserveModules;
        if (!codeSplitting) {
            if (inputOptions_1.manualChunks)
                error({
                    code: 'INVALID_OPTION',
            ...
            return graph_1.buildSingle(inputOptions_1.input).then(function (chunk) {
                ...
                var modules = graph_1.getCache().modules;
                function generate(rawOutputOptions) {
                    ...
                    if (outputOptions.entryNames || outputOptions.chunkNames)
                        error({
                            code: 'INVALID_OPTION',
                            message: '"entryNames" and "chunkNames" options are only supported for code-splitting builds.'
                        });
                    ...
                    var promise = Promise.resolve()
                        ...
                        var inputBase = dirname(resolve(inputOptions_1.input));
                        chunk.exportMode = getExportMode(chunk, outputOptions);
                        chunk.preRender(outputOptions, inputBase);
                        chunk.id = basename(inputOptions_1.input);
                        ...
                        graph_1.plugins.forEach(function (plugin) {
                            if (plugin.ongenerate) {
                                plugin.ongenerate(Object.assign({
                                    bundle: result
                        ...
                        return generate(outputOptions).then(function (result) {
                            ...
                            return (Promise.all(promises)
                                ...
                                return mapSequence(graph_1.plugins.filter(function (plugin) { return plugin.onwrite; }), function (plugin) {
                                    return Promise.resolve(plugin.onwrite(Object.assign({
                                        bundle: result
        ...
        // code splitting case
        if (inputOptions_1.experimentalPreserveModules && typeof inputOptions_1.input === 'string') {
            inputOptions_1.input = [inputOptions_1.input];
        }
        if (!(Array.isArray(inputOptions_1.input) || typeof inputOptions_1.input === 'object')) {
            error({
                code: 'INVALID_OPTION',
        ...
        return graph_1
            .buildChunks(inputOptions_1.input, inputOptions_1.manualChunks, inputOptions_1.experimentalPreserveModules)
            ...
            function generate(rawOutputOptions) {
                ...
                var promise = createAddons(graph_1, outputOptions)
                    .then(function (addons) {
                        ...
                        return Promise.all(Object.keys(result).map(function (chunkName) {
                            ...
                            if (outputOptions.sourcemap) {
                                ...
                                if (outputOptions.sourcemap === 'inline') {
                                    ...
                                    url = chunkName + ".map";
                                    promises.push(writeFile$1(dir + '/' + chunkName + '.map', map.toString()));
                                }
                                code += "//# " + SOURCEMAPPING_URL + "=" + url + "\n";
                            }
                            promises.push(writeFile$1(dir + '/' + chunkName, code));
                            return Promise.all(promises).then(function () {
                                return mapSequence(graph_1.plugins.filter(function (plugin) { return plugin.onwrite; }), function (plugin) {
                                    return Promise.resolve(plugin.onwrite(Object.assign({ bundle: chunk }, outputOptions), chunk));
                                });
                            });
                        })).then(function () { return result; });
                    });
