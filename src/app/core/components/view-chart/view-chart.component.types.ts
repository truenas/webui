...
export interface ChartConfiguration {
   ...
   *
   * Note: When chart is not binded, c3 starts observing if chart.element is binded by MutationObserver.
  ...
  size?: {
     ...
     * If this option is not specified, the width of the chart will be
     * calculated by the size of the parent element it's appended to.
     * Note: This option should be specified if possible because it can improve its performance
     * because some size calculations will be skipped by an explicit value.
     */
    width?: number;
    /**
     * The desired height of the chart element.
     * If this option is not specified, the height of the chart will be calculated
  ...
  transition?: {
     ...
     * Note: If 0 or null set, transition will be skipped.
     * So, this makes initial rendering faster especially in case you have a lot of data.
     */
    duration?: number;
  };
   ...
   * If not specified, the edge values will be used. If timeseries x axis, date string, Date object and
...
export interface Data {
  /**
   * Load a CSV or JSON file from a URL. Note that this will not work if loading
   * via the "file://" protocol as most browsers with block XMLHTTPRequests.
   */
  url?: string;
  /**
   ...
   * We can show the data with non-index x values by this option.
   ...
   * If this option is specified, the element g for the data has an additional class
   * that has the prefix c3-target- (e.g. c3-target-additional-data1-class).
   */
   ...
   * The formatter function receives 4 arguments such as v, id, i, j and
   * it must return a string that will be shown as the label. The arguments are:
   * - v is the value of the data point where the label is shown.
   * - id is the id of the data where the label is shown.
   * - i is the index of the data point where the label is shown.
   * - j is the sub index of the data point where the label is shown.
   * Formatter function can be defined for each data by specifying as an object and D3 formatter
   * function can be set (e.g. d3.format('$'))
   */
  labels?: boolean
   ...
   * This option changes the order of stacking the data and pieces of pie/donut.
   * If null specified, it will be the order the data loaded.
   * If function specified, it will be used to sort the data
   ...
   * If start is not set, the start will be the first data point. If end is not set, the
   * end will be the last data point.
   * Currently this option supports only line chart and dashed style.
   * If this option specified, the line will be dashed only in the regions.
   */
  regions?: { [key: string]: any };

  /**
   * Set color converter function.
   * This option should a function and the specified function receives color (e.g. '#ff0000') and d that
   * has data parameters like id, value, index, etc. And it must return a string that
...
export interface XAxisConfiguration {
   ...
   * If no padding is needed in the ragen of x axis, 0 should be set. On category axis, this option
   * will be ignored.
   */
  padding?: {
    left?: number;
   ...
   * The height of x axis can be set manually by this option. If you need more space for x axis,
   * please use this option for that. The unit is pixel.
   */
  height?: number;
  /**
  ...
  /**
   * Set label on x axis.
   * You can set x axis label and change its position by this option. string and object can be passed and we can
   * change the poisiton by passing object that has position key. Available position
...
export interface YAxisConfiguration {
   ...
   * You can set padding for y axis to create more space on the edge of the axis. This option accepts object and
   * it can include top and bottom. top, bottom will be treated as pixels.
   */
  padding?: {
    top?: number;
    bottom?: number;
...
export interface XTickConfiguration {
   ...
   * This option hides tick lines together with tick text. If this option is used on timeseries axis,
   * the ticks position will be determined precisely and not nicely positioned (e.g. it will
   ...
   * This option works with timeseries data and the x values will be parsed accoding to the type
...
export interface TooltipOptions {
  ...
  format?: {
     ...
     * ratio will be undefined if the chart is not
     * donut/pie/gauge.
     */
