...
import { NonAuthApiDirectory, NonAuthApiMethod } from 'app/interfaces/api-directory.interface';
...
export class MockWebsocketService2 extends WebSocketService2 {
  ...
  constructor(
    ...
    when(this.call).mockImplementation((method: NonAuthApiMethod, args: unknown) => {
      throw Error(`2 Unmocked websocket call ${method} with ${JSON.stringify(args)}`);
    });
    when(this.job).mockImplementation((method: NonAuthApiMethod, args: unknown) => {
      throw Error(`2 Unmocked websocket job call ${method} with ${JSON.stringify(args)}`);
    });
  }

  mockCall<K extends NonAuthApiMethod>(method: K, response: NonAuthApiDirectory[K]['response']): void {
  ...
  mockCallOnce<K extends NonAuthApiMethod>(method: K, response: NonAuthApiDirectory[K]['response']): void {
    when(this.call).calledWith(method, expect.anything()).mockReturnValueOnce(of(response));
  }
  mockJob<K extends NonAuthApiMethod>(method: K, response: Job<NonAuthApiDirectory[K]['response']>): void {
