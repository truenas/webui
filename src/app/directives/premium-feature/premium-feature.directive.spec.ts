...
@Component({
  ...
  template: '<button *ixPremiumFeature="entitled; testId: \'thing\'" class="action" type="button">Do it</button>',
...
describe('PremiumFeatureDirective', () => {
  ...
  // The wrapper's `keepFocusable="ix-tooltip"` has no keyboard half to assert here: on this
  // branch a field's help is a `tn-icon` with no tabindex, which `getFocusableElements` never
  // selects, so the sweep leaves it alone with or without the exemption. What the exemption
  // actually restores is `pointer-events`, which jsdom does not compute — so a test would pass
  // whatever the rule said. Covered by the click-through instead.
});
