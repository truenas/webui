...
import { ConfigResetComponent } from 'app/views/others/config-reset/config-reset.component';
...
describe('ConfigResetComponent', () => {
  ...
  it('replaces location state to avoid resetting config again if user visits the page again', () => {
    expect(spectator.inject(Location).replaceState).toHaveBeenCalledWith('/sessions/signin');
  });
