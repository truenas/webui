...
| `tn-input [inputType]="InputType.Size"` re-parses its own blur-formatted output | On blur the field reformats the value for display and writes that string back through its own parser, so a size that does not survive rounding is silently changed — `2049 KiB` (2098176) redisplays as `2 MiB` and the control drops to 2097152. Typing and `[sizeRound]` both round-trip correctly; only the blur re-parse loses precision | Keep `ix-input` with `[parse]="formatter.memorySizeParsing" [format]="formatter.memorySizeFormatting"` for any size the API needs byte-exact (`iscsi.extent`'s `filesize`; see `extent-form.component.html`). `ix-input` formats for display without re-parsing. Create-only fields where the user types the value anyway are safe on `tn-input` | Library stops feeding its formatted output back through its parser |
